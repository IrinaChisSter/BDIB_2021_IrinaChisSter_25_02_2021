...
-   Let's assume that X, Y are continuous denoted by
    ...
    -   $Cov(X, Y)=\frac{(x_1-\mu_X)\times(y_1-\mu_Y)+(x_2-\mu_X)\times(y_2-\mu_Y)+...(x_n-\mu_X)\times(y_n-\mu_Y)}{}$
        , $Cov(X, Y)=Var(X)$
    ...
    -   \$\\begin{bmatrix} 1 & Cor(X, Y) \\\\ Cor(X, Y) & 1 \\\\
        \\end{bm (S denoted in the R code)

    -   

    -   atrix}\$ variance-covariance matrix
...
##generate/simulate a normal distribution with mean 10 and standard deviation=4)
